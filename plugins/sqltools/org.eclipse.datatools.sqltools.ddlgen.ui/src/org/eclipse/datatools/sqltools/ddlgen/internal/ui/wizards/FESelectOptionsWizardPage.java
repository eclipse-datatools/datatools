...
public class FESelectOptionsWizardPage
	...
	private static final String SELECT_ALL = ResourceLoader.INSTANCE.queryString("SELECT_ALL_BUTTON_TEXT"); //@d00058820gs //$NON-NLS-1$
    private static final String DESELECT_ALL = ResourceLoader.INSTANCE.queryString("DESELECT_ALL_BUTTON_TEXT"); //@d00058820gs //$NON-NLS-1$
    private static final String RESTORE_DEFAULTS = ResourceLoader.INSTANCE.queryString("RESTORE_DEFAULTS_BUTTON_TEXT"); //@d00058820gs //$NON-NLS-1$
...
//        WorkbenchHelp.setHelp(composite, "org.eclipse.wst.rdb.fe.ui.infopop.genddl_wiz_options");
        ...
        for (int index = 0; index < options.length; index++){
        	...
        	if (option != null && option.getCategory().getId().equals(EngineeringOptionCategoryID.GENERATE_OPTIONS)){
        		filteredOptions.add(option);
        	}
        }
        if(filteredOptions.size() == 1) //force all checkboxes to be enabled
            ((EngineeringOption)options[0]).setBoolean(true); 
        ...
        for (int i = 0; i < Math.ceil((double)filteredOptionsArray.length / (double)2.0); i++) //@d00058820gs
        {	
        	addButton (composite1, filteredOptionsArray[i]);
        }
        for (int i = (int)Math.ceil((double)filteredOptionsArray.length / (double)2.0); i < filteredOptionsArray.length; i++) //@d00058820gs
        {
		...
		HelpUtil.setHelp( getControl(), 
				HelpUtil.getContextId(IHelpContextsSQMFEUI.GENERATE_DDL_SELECT_OPTIONS_WIZARD_PAGE, 
