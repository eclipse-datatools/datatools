...
public class TableNavigator extends TableCursor
{
  ...
  private static final String TABLETREEITEM_ID = "TableTreeItemID"; //$NON-NLS-1$
         	...
         	if (!isChild(currentTable, (Control)eventSource))
         	{
         	  if (currentTable.getItemCount() > 0 && currentTable.getSelectionCount() <= 0)
              {
                 ...
                 else
                 {
         	     ...
         	     // Since TableCusor.setSelection() doesn't notify listeners, we
                 // need to do it so listeners are notified of selection
              ...
              !Display.getDefault().getFocusControl().equals(TableNavigator.this))
          {
            ...
            Display.getDefault().asyncExec(new Runnable()            
            {
              ...
              public void run()
              {
                if (!isVisible())
                {
                  // If no row has been selected, make sure there is one
                  // Bugzilla bug 3116
                  if (TableNavigator.this.getRow() == null && 
                      currentTable.getItemCount() > 0 )
                  {
                    currentTable.setSelection(0);                     
                    setSelection(0,0);  
                    // Since TableCusor.setSelection() doesn't notify listeners, we
