...
public class TString
{
   ...
   */
   public static boolean isFileNameFormat(String text)
   {
	 int idx = text.indexOf("."); //$NON-NLS-1$
	 if(idx != -1 && idx != 0 && idx != (text.length()-1))
	   return true;

	 return false;
   }
   /*
   ** check if a hostname is ip address
   **
   */
   public static boolean isIP(String hostName)
   {
		if ( (hostName.startsWith("[")) && (hostName.endsWith("]")) ) //$NON-NLS-1$ //$NON-NLS-2$
		{
			return true;
		}
		else
   ...
   */
   ...
   {
	   String newName = change(name, ".", ""); //$NON-NLS-1$ //$NON-NLS-2$

	   if(isDigits(newName) && newName.length() < name.length())
		 return true;
   ...
   /*
   ** transform an ip host to ip address
   **
   */
	public static String toIP(String ipHost)
	{
		return ipHost.substring(1,ipHost.length()-1);
	}
