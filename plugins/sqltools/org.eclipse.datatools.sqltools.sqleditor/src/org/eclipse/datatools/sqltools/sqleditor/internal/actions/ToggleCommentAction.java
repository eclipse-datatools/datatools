...
public class ToggleCommentAction extends TextEditorAction implements IPageUpdate
{
    ...
    public void run()
    {
        if (_operationTarget == null || _documentPartitioning == null || _prefixesMap == null)
        {
            return;
        }
        ...
        if (!_operationTarget.canDoOperation(operationCode))
        {
            if (shell != null)
            {
                MessageDialog
                    .openError(
                    shell,
                    Messages.ToggleComment_error_title, Messages.ToggleComment_error_message); //$NON-NLS-1$ //$NON-NLS-2$
            }
            return;
        }
        ...
        BusyIndicator.showWhile(display, new Runnable()
        {
            public void run()
            {
                _operationTarget.doOperation(operationCode);
            }
        }
        );
    }
    ...
    private boolean isSelectionCommented(ISelection selection)
    {
        ...
        try
        {
            ...
            ITypedRegion[] regions = TextUtilities.computePartitioning(document, _documentPartitioning, block
            ...
            for (int i = 0, j = 0; i < regions.length; i++, j += 2)
            {
                String[] prefixes = (String[]) _prefixesMap.get(regions[i].getType());
     ...
     * @param document The document
     * @return <code>true</code> iff each line from <code>startLine</code> to and including <code>endLine</code>
     *         is prepended by one of the <code>prefixes</code>, ignoring whitespace at the begin of line
     */
    ...
    public void update()
    {
        ...
        if (_operationTarget == null && editor != null)
        {
            _operationTarget = (ITextOperationTarget) editor.getAdapter(ITextOperationTarget.class);
        }

        boolean isEnabled = (_operationTarget != null && _operationTarget.canDoOperation(ITextOperationTarget.PREFIX) && _operationTarget
