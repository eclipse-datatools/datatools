...
public class GroupSQLResultRunnable extends SimpleSQLResultRunnable {
	private final class ConfirmAction extends Action {
		...
		public void run() {
			...
			IPreferenceStore store = SQLEditorPlugin.getDefault()
					.getPreferenceStore();
			MessageDialogWithToggle dlg = MessageDialogWithToggle
					.openYesNoQuestion(PlatformUI.getWorkbench()
							...
							null/* use default toggle message */, false,
							store, PreferenceConstants.EXECUTE_SQL_ERROR_MODE);
			...
			if (dlg.getToggleState()) {
				...
				// update PreferenceConstants.EXECUTE_SQL_ERROR_MODE to
				// correspond
				store.setValue(PreferenceConstants.EXECUTE_SQL_ERROR_MODE,
						preferenceValue);
			}
			if (result == IDialogConstants.YES_ID) {
				_goon = true;
	 ...
	 * @param tracker
	 */
	public GroupSQLResultRunnable(Connection con, String[] groups,
			IConnectionTracker tracker, Runnable postRun,
			DatabaseIdentifier databaseIdentifier, boolean promptVar,
			HashMap varDefs) {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		if (_groups.length > 1) {
			task += NLS.bind("GroupSQLResultRunnable.groups",
					(new Object[] { "" + _groups.length }));
		}
		...
		try {
			for (int i = 0; i < _groups.length; i++) {
				if (_groups.length > 1) {
					monitor.subTask(NLS.bind("GroupSQLResultRunnable.group",
							(new Object[] { "" + i })));
				}
				...
				_currentJob = f.getExecutionService()
						.createAdHocScriptRunnable(_conn, _groups[i], false,
								_tracker, monitor, getDatabaseIdentifier(),
								null, null);
				if (_currentJob == null) {
					_currentJob = new SimpleSQLResultRunnable(_conn,
							_groups[i], false, _tracker, monitor,
				...
				if (_currentJob instanceof ResultSupportRunnable) {
					...
					try {
						...
						resultsViewAPI
								.appendThrowable(getOperationCommand(), e);
						synchronized (resultSupportRunnable
								.getOperationCommand()) {
							resultsViewAPI
									.appendStatusMessage(resultSupportRunnable
											.getOperationCommand(), e
											.getLocalizedMessage());
							resultsViewAPI.updateStatus(resultSupportRunnable
									.getOperationCommand(),
					...
					if (monitor.isCanceled()) {
					...
					} else if (resultsViewAPI
							.getCurrentStatus(resultSupportRunnable
									.getOperationCommand()) != OperationCommand.STATUS_SUCCEEDED
							&& i < _groups.length - 1) {
						// since we'll kill the connection during terminating,
						// there's no way to continue
						if (resultsViewAPI
								.getCurrentStatus(resultSupportRunnable
										.getOperationCommand()) == OperationCommand.STATUS_TERMINATED) {
							return Status.CANCEL_STATUS;
						}

						IPreferenceStore store = SQLEditorPlugin.getDefault()
								.getPreferenceStore();
						String errorpm = store
								.getString(PreferenceConstants.EXECUTE_SQL_ERROR_MODE);
						if (errorpm == null
								|| errorpm.equals("") || PreferenceConstants.PROMPT_MODE_PROMPT.equals(errorpm)) //$NON-NLS-1$
						{
						...
						} else if (PreferenceConstants.PROMPT_MODE_ALWAYS
								.equals(errorpm)) {
							continue;
						} else {
							// never
