...
//    private static DTPLogger _log = SQLEditorPlugin.getLogger(SQLCommentAutoIndentStrategy.class);
...
//            _log.error(EditorMessages.error_badLocationException, e);
            ...
            if (end >= start)
            {
                ...
                if (end < c.offset)
                {
                    ...
                    if (d.getChar(end) == '/' && d.getChar(end + 1) != '/')
                    {
                        ...
                        PreferenceConstants.SQLEDITOR_CLOSE_COMMENTS)
                            && isNewComment(d, c.offset, _partitioning))
                        {
    ...
    public void customizeDocumentCommand(IDocument document, DocumentCommand command)
    {
        try
        {
            ...
            ITypedRegion partition = TextUtilities.getPartition(document, _partitioning, command.offset, true);
