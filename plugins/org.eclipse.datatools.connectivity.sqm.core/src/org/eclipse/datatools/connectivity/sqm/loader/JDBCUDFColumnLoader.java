...
public class JDBCUDFColumnLoader extends JDBCBaseLoader {
	...
	/**
	 * Loads the "parameter" objects for the UDF. This method uses the result
	 * set from createResultSet() to load the "parameter" objects from the
	 * server. Row handling for the result set is delegated to processRow().
	 * Parameter objects are created using the factory method,
	 * createParameter(). If a result set is returned by the UDF,
	 * createRoutineResultTable() is used to create the container object, while
	 * createColumn() is used to create the columns in the result table.
	 * 
	 ...
	 *         UserDefinedFunction (i.e. (UserDefinedFunction)
	 *         getCatalogObject()).
	 */
	protected UserDefinedFunction getUserDefinedFunction() {
		return (UserDefinedFunction) getCatalogObject();
	}
