/**
 * Created on 2007-1-19
 * 
 * Copyright (c) Sybase, Inc. 2004-2006. All rights reserved.
 */
package org.eclipse.datatools.enablement.sybase;

public interface IGenericDdlConstants
{
    public final static String NEWLINE              = System.getProperty("line.separator"); //$NON-NLS-1$
    public final static String EMPTY_STRING         = ""; //$NON-NLS-1$
    public final static String DOT                  = "."; //$NON-NLS-1$
    public final static String SPACE                = " "; //$NON-NLS-1$
    public final static String COMMA                = ","; //$NON-NLS-1$
    public final static String SINGLE_QUOTE         = "'"; //$NON-NLS-1$
    public final static String DOUBLE_QUOTE         = "\""; //$NON-NLS-1$
    public final static String TAB                  = "\t"; //$NON-NLS-1$
    public final static String LEFT_PARENTHESIS     = "("; //$NON-NLS-1$
    public final static String RIGHT_PARENTHESIS    = ")"; //$NON-NLS-1$
    public final static String DROP                 = "DROP"; //$NON-NLS-1$
    public final static String CREATE               = "CREATE"; //$NON-NLS-1$
    public final static String ALTER                = "ALTER"; //$NON-NLS-1$
    public final static String ADD                  = "ADD"; //$NON-NLS-1$
    public final static String DELETE               = "DELETE"; //$NON-NLS-1$
    public final static String UPDATE               = "UPDATE"; //$NON-NLS-1$
    public final static String CASCADE              = "CASCADE"; //$NON-NLS-1$
    public final static String CASCADED             = "CASCADED"; //$NON-NLS-1$
    public final static String LOCAL                = "LOCAL"; //$NON-NLS-1$
    public final static String OPTION               = "OPTION"; //$NON-NLS-1$
    public final static String RESTRICT             = "RESTRICT"; //$NON-NLS-1$
    public final static String NULL                 = "NULL"; //$NON-NLS-1$
    public final static String NOT                  = "NOT"; //$NON-NLS-1$
    public final static String DEFAULT              = "DEFAULT"; //$NON-NLS-1$
    public final static String SET                  = "SET"; //$NON-NLS-1$
    public final static String TRIGGER              = "TRIGGER"; //$NON-NLS-1$
    public final static String TABLE                = "TABLE"; //$NON-NLS-1$
    public final static String VIEW                 = "VIEW"; //$NON-NLS-1$
    public final static String INDEX                = "INDEX"; //$NON-NLS-1$
    public final static String PROCEDURE            = "PROCEDURE"; //$NON-NLS-1$
    public final static String FUNCTION             = "FUNCTION"; //$NON-NLS-1$
    public final static String CONSTRAINT           = "CONSTRAINT"; //$NON-NLS-1$
    public final static String UNIQUE               = "UNIQUE"; //$NON-NLS-1$
    public final static String CHECK                = "CHECK"; //$NON-NLS-1$
    public final static String TYPE                 = "TYPE"; //$NON-NLS-1$
    public final static String ON                   = "on"; //$NON-NLS-1$
    public final static String FOREIGN_KEY          = "FOREIGN KEY"; //$NON-NLS-1$
    public final static String REFERENCES           = "REFERENCES"; //$NON-NLS-1$
    public final static String PRIMARY_KEY          = "PRIMARY KEY"; //$NON-NLS-1$
    public final static String DEFERRABLE           = "DEFERRABLE"; //$NON-NLS-1$
    public final static String DEFERRED             = "DEFERRED"; //$NON-NLS-1$
    public final static String INITIALLY            = "INITIALLY"; //$NON-NLS-1$
    public final static String ALIAS                = "ALIAS"; //$NON-NLS-1$
    public final static String AS                   = "AS"; //$NON-NLS-1$
    public final static String FOR                  = "FOR"; //$NON-NLS-1$
    public final static String LONG                 = "LONG"; //$NON-NLS-1$
    public final static String BLOB                 = "BLOB"; //$NON-NLS-1$
    public final static String DBCLOB               = "DBCLOB"; //$NON-NLS-1$
    public final static String CLOB                 = "CLOB"; //$NON-NLS-1$
    public final static String VARCHAR              = "VARCHAR"; //$NON-NLS-1$
    public final static String WITH                 = "WITH"; //$NON-NLS-1$
    public final static String COMPARISONS          = "COMPARISONS"; //$NON-NLS-1$
    public final static String DATALINK             = "DATALINK"; //$NON-NLS-1$
    public final static String VARGRAPHIC           = "VARGRAPHIC"; //$NON-NLS-1$
    public final static String AFTER                = "AFTER"; //$NON-NLS-1$
    public final static String BEFORE               = "BEFORE"; //$NON-NLS-1$
    public final static String INSTEAD_OF           = "INSTEAD OF"; //$NON-NLS-1$
    public final static String INSERT               = "INSERT"; //$NON-NLS-1$
    public final static String NO                   = "NO"; //$NON-NLS-1$
    public final static String OF                   = "OF"; //$NON-NLS-1$
    public final static String REFERENCING          = "REFERENCING"; //$NON-NLS-1$
    public final static String NEW                  = "NEW"; //$NON-NLS-1$
    public final static String OLD                  = "OLD"; //$NON-NLS-1$
    public final static String NEW_TABLE            = "NEW_TABLE"; //$NON-NLS-1$
    public final static String OLD_TABLE            = "OLD_TABLE"; //$NON-NLS-1$
    public final static String EACH                 = "EACH"; //$NON-NLS-1$
    public final static String ROW                  = "ROW"; //$NON-NLS-1$
    public final static String STATEMENT            = "STATEMENT"; //$NON-NLS-1$
    public final static String WHEN                 = "WHEN"; //$NON-NLS-1$
    public final static String RENAME               = "RENAME";
    public final static String MODIFY               = "MODIFY"; 
}
