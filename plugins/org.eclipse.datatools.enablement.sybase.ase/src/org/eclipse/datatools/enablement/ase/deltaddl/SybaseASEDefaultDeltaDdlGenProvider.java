...
public class SybaseASEDefaultDeltaDdlGenProvider extends AbstractDeltaDdlGenProvider implements IDeltaDdlGenProvider,
            ...
            boolean qualifyNames, boolean fullSyntax, SybaseDdlScript script, IProgressMonitor monitor)
    {
        ...
        if (list != null)
        {
            for (int i = 0; i < list.size(); i++)
            {
                ...
                if (crFeatureID == EcorePackage.EATTRIBUTE__NAME && !cr.oldValue.equals(cr.newValue))
                {
