 ...
 * TODO: ckadner finish! please crab me for that! SORRY -- time's short!
 * 
 * 
 *
 * TODO: clear up comment 
...
public class SQLQuerySourceWriter implements SQLSourceWriter
{
    ...
    /**
     * Generic method to return the SQL source of ther given
    ...
    public String getSQL(SQLQueryObject queryObject)
    {
        ...
        if (queryObject != null)
        {
            ...
            // or is being called purposly on a subclass of , by extendind the base SourceWriter
        	try
            {
...
//	                            + queryObject.getClass().getName());
    ...
    /**
     * Helper method to append the given number of spaces (' ') to the given
     * StringBuffer sb.
     */
    ...
    protected void appendSpecificSQL(SearchConditionCombinedOperator op, StringBuffer sb)
    {
        ...
        // TODO superclassing somewhere
        // StringBuffer sbsuper;
        //appendSQL(op.getClass().getSuperclass(),sbsuper, parent);
...
*/
    ...
    {
        // e.g. datatype "VARCHAR (256) FOR BIT DATA" has as name
        // "VARCHAR () FOR BIT DATA" so we have to insert the length in the parens

        StringBuffer lengthSB = new StringBuffer();

        if (dataType.getLength() > 0)
        {
            lengthSB.append(PAREN_LEFT);
    ...
    /** computes the size of a CLOB or BLOB with K,M,G byte unit specifier */
...
*/
    ...
    /**
     * Appends new-line <code>character</code> ({@link #NEW_LINE}) to the given
     * StringBuffer <code>sb</code> only if the last line does not already
     * end with a new-line <code>character</code>.
     *
     * @param sb
     */
...
*/
    ...
    protected void appendSpecificSQL(QuerySelectStatement selectStmt, StringBuffer sb)
    {
        if (selectStmt.getQueryExpr() != null) {
            ...
            appendSQLForOrderByClause(selectStmt.getOrderByClause(), sb);
        } else {
...
/*    protected boolean isLastNonWhiteSpaceCharEqual(char c, StringBuffer sb)
    {
        boolean lastCharEqual = false;
        for (int i = sb.length()-1; i >= 0; i--)
        {
            char lastChar = sb.charAt(i);
            if (lastChar != SPACE && lastChar != NEW_LINE)
            {
                lastCharEqual = (c == lastChar);
                break;
            }
        }
        return lastCharEqual;
    }
*/
    /**
    ...
    protected void appendSQLForOrderByClause(List orderByClause, StringBuffer sb)
    {
        ...
        if (StatementHelper.isOrderByClauseContainsValidOrderBySpecification(orderByClause))
        {
            ...
            sb.delete(sb.length() - STANDARD_INDENT, sb.length());
        }
    }
...
*/
    ...
    protected void appendSpecificSQL(TableInDatabase tableInDB, StringBuffer sb)
    {
        ...
        if (tableInDB.getTableCorrelation() != null )
        {
            ...
            sb.append(AS);
            sb.append(SPACE);
            appendSQL(tableInDB.getTableCorrelation(),sb);
        }
    }
    ...
    {
        sb.append(StatementHelper.convertCatalogIdentifierToSQLFormat(withTableRef.getName(), getDelimitedIdentifierQuote()));
        ...
        if (withTableRef.getTableCorrelation() != null)
        {
            ...
            sb.append(AS);
            sb.append(SPACE);
            appendSQL(withTableRef.getTableCorrelation(),sb);
        }
    }
    ...
    protected void appendSpecificSQL(WithTableSpecification withTable, StringBuffer sb) 
    {
        ...
        sb.append(StatementHelper.convertCatalogIdentifierToSQLFormat(withTable.getName(), getDelimitedIdentifierQuote()));
...
/*        if (tableRef instanceof TableInDatabase)
        {
            TableInDatabase dbTable = (TableInDatabase) tableRef;
            appendSQL(dbTable, sb);
        }
        else if (tableRef instanceof TableNested)
        {
            TableNested nestedTable = (TableNested) tableRef;
            appendSQL(nestedTable, sb);
        }
        else if (tableRef instanceof TableJoined)
        {
            TableJoined joinedTable = (TableJoined) tableRef;
            appendSQL(joinedTable, sb);
        }
        else */
    ...
    protected void appendSpecificSQL(ValueExpressionColumn column, StringBuffer sb)
    {
            ...
            if (qualify)
            {
                ...
                else // no table alias given
                {
                    ...
                    if (tableExpr instanceof TableInDatabase) {
                        ...
                        sb.append(StatementHelper.convertCatalogIdentifierToSQLFormat(tableExpr.getName(), getDelimitedIdentifierQuote()));
                    }
        ...
        sb.append(StatementHelper.convertCatalogIdentifierToSQLFormat(column.getName(), getDelimitedIdentifierQuote()));

    }

    /**
     * Determins if the given <code>column</code> ought to be qualified
     * according to the parameters specified or if it is ambiguous.
     * 
     * @param alwaysQualifyColumnNamesForMultipleTables
     * @param qualifyColumnNamesInSubqueriesWhenQualifiedInSuperQuery
     * @param alwaysQualifyColumnNamesForSubqueries
     * @param alwaysQualifyColumnNamesReferencedInSubqueries
     * 
     * @param column
     * @return
     */
    private boolean isQualifiedColumnNameRequired(ValueExpressionColumn column)
    {
        ...
        if (alwaysQualifyColumnNamesForMultipleTables)
        {
            // qualify always if there is more than one table in the FROM-
            // clause
            QuerySelect colTableSelect = (QuerySelect) StatementHelper.getEContainerRecursively(column, QuerySelect.class);
            List fromTables = StatementHelper.getTableExpressionsInQuerySelect(colTableSelect);
            multipleTables = fromTables != null && fromTables.size() > 1;
            
            if (qualifyColumnNamesInSubqueriesWhenQualifiedInSuperQuery)
            {
                // do we have multiple tables in super query? be consistent!
                // e.g. in "SELECT T1.COL1, T3.COL3 FROM T1, T3 WHERE EXISTS (SELECT COL2 FROM T2 WHERE T1.COL1 = T2.COL2)"
                // get this containing (sub)query
                QuerySelect subselect = (QuerySelect) StatementHelper.getEContainerRecursively(column, QuerySelect.class);
                // get superior query
                QuerySelect superSelect = (QuerySelect) StatementHelper.getEContainerRecursively(subselect, QuerySelect.class);
                // do we have super query?
                if (superSelect != null)
                {
        ...
        if (alwaysQualifyColumnNamesForSubqueries)
        {
            // qualify always if there is a superior query to distinguish between
            // cloumns in scope of the subquery and columns of superior scope
            // e.g. in "SELECT *  FROM T1 WHERE EXISTS (SELECT * FROM T2 WHERE T1.COL1 = T2.COL2)"
            ...
            subquery = superSelect != null;
        }
        
        if (alwaysQualifyColumnNamesReferencedInSubqueries)
        {
            // qualify all columns that are referenced in a subquery
            // e.g. "T1.COL1" in "SELECT COL1 FROM T1 WHERE EXISTS (SELECT * FROM T2 WHERE T1.COL1 = COL2)"
            
            // get the select of the occurence of the column reference
            QuerySelect colRefSelect = (QuerySelect) StatementHelper.getEContainerRecursively(column, QuerySelect.class);
            // a column contained in OrderBySpecification has no QuerySelect as container, but can also never be in a subselect
                  
            // get the select that contains the table the the column belongs to
            QuerySelect colTableSelect = StatementHelper.getQuerySelectForTableReference(column.getTableExpr());
            referenceInSubquery =
                colRefSelect != null && colTableSelect != colRefSelect;
        }
        
        ambiguousColumnNames = StatementHelper.isColumnNameAmbiguous(column);
        
        qualify = ambiguousColumnNames 
            || (alwaysQualifyColumnNamesForMultipleTables && multipleTables)
            || (alwaysQualifyColumnNamesForSubqueries && subquery)
            || (alwaysQualifyColumnNamesReferencedInSubqueries && referenceInSubquery);
